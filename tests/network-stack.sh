...
grep -Fq 'enable_profile_local_system_service "$profile_copy" host-network-online.service' "$REPO_DIR/scripts/build-iso.sh" || fail "live ISO does not enable aggregate network wait"
